...
保护参数可以控制工作表开启保护后，用户依然可以进行的操作，默认全部开启。单元格编辑权限由 cell.lo 和 编辑区域控制。
默认有三个条件控制单元格是否可编辑：
- 开启工作表保护，单元格默认是锁定状态，锁定属性在 `cell.lo = 不存在 | 1`
- 开启保护界面，允许用户编辑区域配置：设置后，单元格可编辑，默认还是锁定状态
- 单元格格式，关闭“锁定单元格”：用户可以编辑
------------
